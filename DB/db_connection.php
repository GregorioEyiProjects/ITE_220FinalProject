...
//Authenticate user
function userAuthentication($username, $hashed_password){
    global $conn, $get_user_credentials;
    $custom_query = $conn->prepare($get_user_credentials);
    $custom_query->bind_param("ss", $username, $hashed_password);
    $custom_query->execute();
    $result = $custom_query->get_result();
    $row = $result->fetch_assoc();
    return $row;
